...
| **Efficiency** | Gemini Flash with 1M-token context (no RAG/vector DB needed); decision-map **and** comparison results cached per document so repeat opens never re-call Gemini; citation validator normalizes each page once (O(pages+quotes), not O(pages×quotes)); memoized React derivations + throttled streaming; WebP assets with immutable long-cache headers; gzip on, `poweredByHeader` off; Next.js standalone build |
...
| **Accessibility** | Semantic HTML5 (`<main>`, `<section>`, `<nav>`), ARIA (`aria-live`, `aria-selected`, `role="tablist"`, `aria-busy`), AA-contrast tokens (≥4.5:1 in both themes), `lang` attributes on translated output so screen readers pronounce Hindi/Tamil/… correctly (WCAG 3.1.2), keyboard-navigable with visible focus rings, dark mode via `prefers-color-scheme`, `prefers-reduced-motion` support, skip-to-content link |
